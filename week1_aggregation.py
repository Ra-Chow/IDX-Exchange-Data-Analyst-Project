...
filePathPatternListings = "/Users/raineechow/Documents/IDX Exchange/csv/CRMLSListing*.csv"
filePathPatternSold = "/Users/raineechow/Documents/IDX Exchange/csv/CRMLSSold*.csv"
...
filteredListings = combinedListings[combinedListings['PropertyType'] == 'Residential']
finalListingsCount = len(filteredListings)

# Save the filtered listings to a new CSV file.
filteredListings.to_csv("week1_residential_listings.csv", index=False)
...
# Concatenate the sold CSV files into a single DataFrame.
combinedSold = [pd.read_csv(f) for f in soldFiles]
combinedSoldDF = pd.concat(combinedSold, ignore_index=True)
...
filteredSold.to_csv("week1_residential_sold.csv", index=False)
# --------------------------------------------------
...
print(f"Initial number of sold properties: {len(combinedSoldDF)}")
